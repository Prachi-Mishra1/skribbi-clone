...
class MessageHandler {
    ...
    handle(socket) {
        ...
        socket.on("create_room", (data) => {
            const room = new Room_1.Room(this.io, data.settings || {}, data.isPrivate || false);
        ...
        socket.on("join_room", (data) => {
            ...
            if (room.players.size >= room.game.settings.maxPlayers) {
        ...
        socket.on("word_chosen", (data) => {
            ...
            const blanks = data.word.split("").map((c) => c === " " ? " " : "_").join(" ");
        ...
        socket.on("draw_start", (data) => {
            ...
            if (!room)
                return;
            if (!room.getPlayerBySocketId(socket.id)?.isDrawing)
                return;
        ...
        socket.on("chat", (data) => {
            ...
            if (!player)
                return;
            if (player.isDrawing && room.game.phase === "drawing")
                return;
